...
function ModalCancelacion({ boleto, onCerrar, onCancelado }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 sm:p-4" onClick={onCerrar}>
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5">
          {exito ? (
            <div className="text-center">
              ...
              <p className="mb-1 text-lg font-black text-slate-800">Boleto cancelado</p>
            ...
            <div>
              <div className="mb-4 flex items-start gap-2 rounded-2xl! bg-red-50 px-4 py-3 text-sm font-semibold text-red-600">
            ...
            <>
              ...
              <div className="flex gap-2 sm:gap-3">
                ...
                <button onClick={cancelar} disabled={procesando}
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-2xl! bg-red-600 py-3 text-sm font-bold text-white hover:bg-red-500 disabled:opacity-70">
  ...
  return (
    <div className="min-h-screen p-3 sm:p-4 md:p-6">
      <div className="mx-auto">
        ...
        {/* Cabecera: título + botón en la misma fila siempre */}
        <div className=" flex items-start justify-between gap-3 ">
          <div className="min-w-0">
            <h1 className="sm:text-sm  leading-tight text-slate-900 ">
          ...
          <button onClick={cargar}
            className="flex shrink-0 items-center gap-1.5 rounded-xl! border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:border-teal-300 sm:gap-2 sm:px-4 sm:py-2.5 sm:text-sm">
            <RefreshCw size={13} className="sm:size-4 " />
          </button>
        </div>

        {error && (
          <div className="mb-4 flex items-center gap-2 rounded-2xl! bg-red-50 px-4 py-3 text-sm font-semibold text-red-600 sm:mb-5">
            <AlertTriangle size={16} />{error}
          </div>
        )}

        {/* KPIs: texto más compacto en móvil */}
        <div className=" grid lg:grid-cols-3 gap-2  sm:gap-1 sm:grid-cols-1 mb-3">
          {[
            ...
            <div key={k.label} className="rounded-3xl! border border-slate-100 bg-white lg:p-1 text-center shadow-sm  ">
              <p className={`text-xl font-black sm:text-xl ${k.color}`}>{k.valor}</p>
        ...
        {/* Filtros */}
        <div className="mb-3 flex gap-1 rounded-2xl! border border-slate-200 bg-white p-1 sm:mb-4">
          {[
            { v: 'todos',      l: 'Todos'      },
            { v: 'activos',    l: 'Activos'    },
            ...
            <button key={f.v} onClick={() => setFiltro(f.v)}
              className={`flex-1 rounded-xl! py-1.5 text-xs font-bold transition sm:py-2 sm:text-sm
        ...
        {cargando ? (
          ...
          <div className="space-y-3">
            {filtrados.map(b => (
              <div key={b.id} className={`overflow-hidden rounded-3xl! border bg-white shadow-sm
                ${b.estado === 'Cancelado' ? 'border-slate-100 opacity-70' : 'border-slate-100'}`}>

                {/* Header */}
                <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-3 py-2 sm:px-5 sm:py-3">
                  <span className="text-xs font-bold text-slate-500">{b.codigo_boleto}</span>
                  <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-bold sm:gap-1.5 sm:px-2.5 sm:py-1 ${badgeBoleto(b.estado)}`}>
                ...
                <div className="px-3 py-3 sm:px-5 sm:py-4">
                  {/* Timeline horas — whitespace-nowrap para que no se partan */}
                  <div className="flex items-center gap-2">
                    <div className="shrink-0">
                      <p className="whitespace-nowrap text-lg font-black leading-none text-slate-900 sm:text-xl">
                        {new Date(b.fecha_hora_salida).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit', hour12: false })}
                      </p>
                    ...
                    <div className="shrink-0 text-right">
                      <p className="whitespace-nowrap text-lg font-black leading-none text-slate-900 sm:text-xl">
                        {b.fecha_hora_llegada
                          ? new Date(b.fecha_hora_llegada).toLocaleTimeString('es-BO', { hour: '2-digit', minute: '2-digit', hour12: false })
                          : '--:--'}
                      </p>
                  ...
                  {/* Meta info: apilada en móvil, inline en sm */}
                  <div className="mt-2 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-slate-400">
                ...
                {puedesCancelar(b) && (
                  <div className="border-t border-slate-100 px-3 py-2.5 sm:px-5 sm:py-3">
                    <button onClick={() => setModal(b)}
                      className="flex items-center gap-1.5 text-sm font-bold text-red-500 hover:text-red-600">
                      <XCircle size={14} /> Cancelar este boleto
                    </button>
                  </div>
                )}
              </div>
